...
export default class ProfileTrainer extends Component {
    ...
    async componentDidMount(){
        ...
        const trainers = await axios.get('http://backend-sic-gym-uptc.herokuapp.com/api/trainerlogin/')
        this.setState({
            idTrainer: trainers.data[trainers.data.length - 1].idTrainer
        })
        const trainer = await axios.get('http://backend-sic-gym-uptc.herokuapp.com/api/trainers/' + this.state.idTrainer)
